...
From https://github.com/dparrini/python-comtrade with patches.
Should make a pull request.

Dylan Tarter EDITS:
    - Added lots of properties functions for settings properties later,
    search for '### NEW' to see added functions
    - Added some functions for comtrade writing and conversion
         - only supports writing in float32 format
         - does not support digital channels
...
re_dt = re.compile("([0-9]{1,2})/([0-9]{1,2})/([0-9]{4}),([0-9]{2}):([0-9]{2}):([0-9]{2})\\.([0-9]{3,12})")
...
class Cfg:
    ...
    def channels_count(self, val):
        self._channels_count = val
    ...
    ## NEW
    @analog_count.setter
    def analog_count(self, val):
        self._analog_count = val
...
class Comtrade:
    ...
    def cfg_summary(self):
        ...
        st = st + "From {} to {} with time mult. = {}\n".format(self.start_timestamp, self.trigger_timestamp,
                                                                self._cfg.timemult)
        st = st + "{} format\n".format(self.ft)
        return st
...
class BinaryDatReader(DatReader):
    ...
    def parse(self, contents):
        ...
        bufsiz = struct.calcsize(self.get_reader_format(achannels, dbytes))
        #       print ('achannels', achannels)
        #       print ('schannels', schannel)
        #       print ('sample_id_bytes', sample_id_bytes)
        #       print ('abytes', abytes)
        #       print ('dbytes', dbytes)
        #       print ('bytes_per_row', bytes_per_row)
        #       print ('groups_of_16bits', groups_of_16bits)
        #       print ('buffer', self.get_reader_format(achannels, dbytes))
        #       print ('calcsize', bufsiz)
        #       print ('struct.size', rowreader.size)
        #       print ('II', struct.calcsize('II'))
        #       print ('26i', struct.calcsize('26i'))
        #       print ('20H', struct.calcsize('20H'))
...
class Binary32DatReader(BinaryDatReader):
    ...
    STRUCT_FORMAT = "II {acount:d}i {dcount:d}H"
    STRUCT_FORMAT_ANALOG_ONLY = "II {acount:d}i"
...
"""Below are custom functions from Dylan Tarter for writing comtrade"""
import numpy as np
from collections import deque

"""Converts a read comtrade file to a different file type"""


def ComtradeConverter(filename, mode):
    # uses comtrade.py to read input comtrade
    rec = Comtrade()
    rec.load(filename + '.cfg', filename + '.dat')

    numA = len(rec.analog)
    numD = len(rec.status)

    # for each analog channel, adjust the CFG entry
    for i in range(numA):
        if (mode != rec.cfg._ft):
            if (mode == 'ASCII'):
                print('write mode not implemented')
            elif (mode == 'binary'):
                print('write mode not implemented')
            elif (mode == 'binary32'):
                print('write mode not implemented')
            elif (mode == 'float32'):
                # float32's have no scaling
                rec.cfg.analog_channels[i].a = 1
                rec.cfg.analog_channels[i].b = 0
                rec.cfg.analog_channels[i].cmin = float(-3.402823466E+38)
                rec.cfg.analog_channels[i].cmax = float(3.402823466E+38)
            else:
                print('mode invalid')

    writeCFG(filename, mode, rec)
    writeDAT(filename, mode, rec)


"""Similar to convert but just assumes that the config is already correctly formatted"""


def ComtradeWrite(filename, mode, rec):
    writeCFG(filename, mode, rec)
    writeDAT(filename, mode, rec)
...
def writeCFG(filename, mode, rec):
    # write header info to config ASCII file
    fout = open(filename + '.cfg', 'wt')
    fout.write(','.join([rec.cfg._station_name, rec.cfg._rec_dev_id, '2013']) + '\n')
    fout.write(
        str(rec.cfg._channels_count) + ', ' + str(rec.cfg._analog_count) + 'A, ' + str(rec.cfg._status_count) + 'D\n')

    # write each analog channel
    for i in range(rec.cfg.analog_count):
        fout.write(str(rec.cfg.analog_channels[i]) + '\n')
    for i in range(rec.cfg.status_count):
        fout.write(str(rec.cfg.status_channels[i]) + '\n')

    # write the nom. frequency and num of rates
    fout.write(str(int(rec.cfg._frequency)) + '\n')
    fout.write(str(int(rec.cfg._nrates)) + '\n')

    # write each sample rate
    for rates in rec.cfg._sample_rates:
        fout.write(str(int(rates[0])) + ',' + str(int(rates[1])) + '\n')

    # write extra info about timestamps and time codes etc.
    fout.write(rec.cfg._start_timestamp.strftime("%d/%m/%Y,%H:%M:%S.%f") + '\n')
    fout.write(rec.cfg._trigger_timestamp.strftime("%d/%m/%Y,%H:%M:%S.%f") + '\n')
    fout.write(mode + '\n')
    fout.write(str(rec.cfg._timemult) + '\n')
    fout.write(str(int(rec.cfg._time_code)) + ',' + str(int(rec.cfg._local_code)) + '\n')
    fout.write(str(int(rec.cfg._tmq_code)) + ',' + str(int(rec.cfg._leapsec)) + '\n')

    fout.close()


"""Write the comtrade .dat file, only works for float32 currently."""


def writeDAT(filename, mode, rec):
    numA = len(rec.analog)
    numD = len(rec.status)
    # write the .dat file in the output mode
    if (mode == 'ASCII'):
        print('unsupported mode!')
    else:
        fout = open(filename + '.dat', 'wb')
        ...
        if (mode == 'binary'):
            ...
            if (numD > 0):
                print('WARNING!!! Status Channel Packing NOT Implemented, skipping status channels.')

            # format to write is 2 integers and numA floats
            fmt = '<2I%uf'
            fmt = fmt % numA
            A = np.array(rec.analog)

            # write each row
            for k in range(len(rec.time)):
                line = deque(list(A[:, k]))
                line.appendleft(int(rec.time[k] / rec.cfg._time_base))
                line.appendleft(int(k + 1))
                fout.write(struct.pack(fmt, *line))
